...
function messagesUrl(baseUrl) {
  return joinPath(baseUrl, "/messages");
}
...
async function passthroughResponses({ profile, apiKey, req, res, route, body, throwOnRetriableStatus }) {
  const requestBody = body || (await pipeRequestBody(req));
  const startedAt = Date.now();
  const response = await fetchWithRetry(responsesUrl(profile.baseUrl), {
    method: "POST",
...
async function passthroughResponsesCompact({ profile, apiKey, req, res, route, body, throwOnRetriableStatus }) {
  const requestBody = body || (await pipeRequestBody(req));
  const startedAt = Date.now();
  const response = await fetchWithRetry(responsesCompactUrl(profile.baseUrl), {
    method: "POST",
    headers: openaiRequestHeaders(req, apiKey),
    body: requestBody,
  }, { retries: 0, timeoutMs: 300000 });
...
async function fetchChatCompletions({ profile, apiKey, req, body, retries = 0, timeoutMs = 300000 }) {
  return fetchWithRetry(chatCompletionsUrl(profile.baseUrl), {
    method: "POST",
    headers: openaiRequestHeaders(req, apiKey),
    body,
  }, { retries, timeoutMs });
...
async function passthroughMessages({ profile, apiKey, req, res, body, throwOnRetriableStatus }) {
  const requestBody = body || (await pipeRequestBody(req));
  const startedAt = Date.now();
  const response = await fetchWithRetry(messagesUrl(profile.baseUrl), {
    method: "POST",
