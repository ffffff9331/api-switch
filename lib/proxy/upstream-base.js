...
async function fetchAnthropicMessages({ profile, apiKey, req, body, retries = 0, timeoutMs = 300000 }) {
  return fetchWithRetry(messagesUrl(profile.anthropicBaseUrl || profile.baseUrl), {
    method: "POST",
...
async function passthroughMessages({ profile, apiKey, req, res, body, throwOnRetriableStatus }) {
  const requestBody = body || (await pipeRequestBody(req));
  const startedAt = Date.now();
  const response = await fetchAnthropicMessages({ profile, apiKey, req, body: requestBody });
  const upstreamMs = Date.now() - startedAt;
  if (throwOnRetriableStatus && response.status >= 500) {
    await response.arrayBuffer();
