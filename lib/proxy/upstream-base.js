...
function fetchErrorReason(error) {
  ...
  if (error && error.cause && error.cause.message && error.cause.message !== error.message) parts.push(error.cause.message);
  return Array.from(new Set(parts.filter(Boolean))).join(": ");
}

function upstreamFetchError(error, url, attempt, retries) {
  const target = new URL(url);
  const reason = fetchErrorReason(error);
  const message = [
    `Upstream connection failed before response headers from ${target.origin}${target.pathname}`,
    reason ? ` (${reason})` : "",
    retries ? ` after ${attempt + 1}/${retries + 1} attempt(s)` : "",
    ".",
  ].join("");
...
async function fetchWithRetry(url, init, options = {}) {
  ...
  const retryStatuses = options.retryStatuses !== false;
  let lastError;
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    try {
      const response = await fetchWithTimeout(url, init, options);
      const shouldRetry = retryStatuses && (response.status >= 500 || response.status === 429);
      ...
      lastError = error;
      if (attempt === retries) throw upstreamFetchError(error, url, attempt, retries);
      await sleep(Math.min(250 * (attempt + 1), 1000));
    }
  }
  throw lastError ? upstreamFetchError(lastError, url, retries, retries) : new Error("Upstream request failed.");
}
...
async function passthroughResponses({ profile, apiKey, req, res, route, body, throwOnRetriableStatus, debugDir, traceId, streamingFirstChunkTimeoutMs }) {
  ...
  let response = await fetchWithRetry(upstreamUrl, {
    ...
    body: requestBody,
  }, { retries: 1, retryStatuses: false, timeoutMs: 300000 });
  ...
  if (response.ok && response.headers.get("content-type") && response.headers.get("content-type").includes("text/event-stream")) {
    response = await primeStreamingResponse(response, streamingFirstChunkTimeoutMs || 60000, "upstream Responses stream");
  }
...
async function passthroughResponsesCompact({ profile, apiKey, req, res, route, body, throwOnRetriableStatus, debugDir, traceId, streamingFirstChunkTimeoutMs }) {
  const requestBody = body || (await pipeRequestBody(req));
  const startedAt = Date.now();
  ...
  if (response.ok && response.headers.get("content-type") && response.headers.get("content-type").includes("text/event-stream")) {
    response = await primeStreamingResponse(response, streamingFirstChunkTimeoutMs || 60000, "upstream Responses compact stream");
  }
...
async function fetchChatCompletions({ profile, apiKey, req, body, retries = 1, timeoutMs = 300000 }) {
  return fetchWithRetry(chatCompletionsUrl(profile.chatBaseUrl || profile.baseUrl), {
    method: "POST",
    headers: openaiRequestHeaders(req, apiKey),
    body,
  }, { retries, retryStatuses: false, timeoutMs });
}

async function fetchCompletions({ profile, apiKey, req, body, retries = 1, timeoutMs = 300000 }) {
...
async function passthroughMessages({ profile, apiKey, req, res, body, throwOnRetriableStatus, streamingFirstChunkTimeoutMs }) {
  const requestBody = body || (await pipeRequestBody(req));
  const startedAt = Date.now();
  let response = await fetchWithRetry(messagesUrl(profile.anthropicBaseUrl || profile.baseUrl), {
    method: "POST",
  ...
  if (response.ok && response.headers.get("content-type") && response.headers.get("content-type").includes("text/event-stream")) {
    response = await primeStreamingResponse(response, streamingFirstChunkTimeoutMs || 60000, "upstream Messages stream");
  }
