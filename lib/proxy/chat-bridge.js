...
const { fetchChatCompletions, passthroughMessages, passthroughResponses } = require("./upstream-base");
...
function responsesInputToMessages(body) {
  ...
  if (Array.isArray(input)) {
    for (const item of input) {
      ...
      const content = chatContentFromResponsesContent(item.content || item.text || item.input_text || "");
...
function responsesToChatPayload(body, model) {
  const payload = {
    model,
...
async function proxyResponsesViaChat(args) {
  ...
  const chatPayload = responsesToChatPayload(body, model);
  ...
  if (response.status === 404 || response.status === 405 || (args.route && args.route.family === "openai" && response.status === 400)) {
...
async function proxyMessagesViaChat(args) {
  ...
  const upstreamBody = Buffer.from(JSON.stringify(chatPayload), "utf8");
  const response = await fetchChatCompletions({ profile: args.profile, apiKey: args.apiKey, req: args.req, body: upstreamBody });
  if (response.status === 404 || response.status === 405) return passthroughMessages(args);
  if (chatPayload.stream && response.ok && response.headers.get("content-type") && response.headers.get("content-type").includes("text/event-stream")) {
