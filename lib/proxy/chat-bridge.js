...
function responsesToChatPayload(body, model, options = {}) {
  ...
  else if (body.stream === true) payload.stream_options = { include_usage: true };
  if (body.response_format !== undefined) payload.response_format = body.response_format;
  for (const key of ["metadata", "seed", "service_tier", "logprobs", "top_logprobs", "logit_bias", "n"]) {
    if (body[key] !== undefined) payload[key] = body[key];
  }
  if (body.reasoning_effort !== undefined) payload.reasoning_effort = body.reasoning_effort;
  else if (body.reasoning && typeof body.reasoning === "object" && body.reasoning.effort !== undefined) payload.reasoning_effort = body.reasoning.effort;
  if (body.reasoning !== undefined) payload.reasoning = body.reasoning;
