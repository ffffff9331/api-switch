...
const { chatCompletionsUrl, completionsUrl, fetchAnthropicMessages, fetchChatCompletions, fetchCompletions, passthroughMessages, passthroughResponses, passthroughResponsesCompact } = require("./upstream-base");
...
function responsesToAnthropicPayload(body, model) {
  const messages = [];
  if (body.instructions) messages.push({ role: "user", content: String(body.instructions) });
  const inputMessages = responsesInputToMessages(body, { textOnlyImages: true });
  for (const message of inputMessages) {
    if (!message || message.role === "system") continue;
    const role = message.role === "assistant" ? "assistant" : "user";
    const content = typeof message.content === "string" ? message.content : textFromResponsesInput(message.content);
    if (content) messages.push({ role, content });
  }
  return {
    model,
...
function anthropicToResponses(message, model) {
  const responseId = id("resp");
  const output = [];
  const text = extractAnthropicText(message && message.content);
  if (text) {
    output.push({
      id: id("msg"),
      type: "message",
...
function extractAnthropicText(content) {
  ...
  return content.map((part) => {
    ...
    return textFromContentPart(part);
  }).filter(Boolean).join("");
}

function proxyResponsesViaAnthropicMessages(args) {
...
async function proxyResponsesViaAnthropic(args) {
  ...
  const anthropicPayload = responsesToAnthropicPayload(body, model);
  const upstreamBody = Buffer.from(JSON.stringify(anthropicPayload), "utf8");
  const response = await fetchAnthropicMessages({ profile: args.profile, apiKey: args.apiKey, req: args.req, body: upstreamBody });
  ...
  if (!response.ok) {
    if (args.throwOnRetriableStatus && response.status >= 500) {
      const error = new Error(`Upstream anthropic messages failed with ${response.status}: ${text}`);
  ...
  try {
    ...
    args.res.end(JSON.stringify({ error: { message: `Failed to parse anthropic messages response: ${error.message}` } }));
    return;
  }
  const payload = anthropicToResponses(message, model);
  if (body.stream) return writeResponsesPayloadAsSse(args.res, payload, "anthropic-messages-bridge");
  args.res.writeHead(200, {
    "content-type": "application/json; charset=utf-8",
    "x-api-switch-upstream-protocol": "anthropic-messages-bridge",
...
function writeResponsesPayloadAsSse(res, payload, protocol = "chat-completions-bridge") {
  const response = payload || {};
  res.writeHead(200, {
    "content-type": "text/event-stream; charset=utf-8",
    "cache-control": "no-cache",
    connection: "keep-alive",
    "x-api-switch-upstream-protocol": protocol,
  });
...
async function proxyResponsesViaChat(args) {
  ...
  const response = await fetchChatCompletions({ profile: args.profile, apiKey: args.apiKey, req: args.req, body: upstreamBody });
...
async function proxyMessagesViaChat(args) {
  ...
  if (args.profile && args.profile.claudeUpstreamProtocol === "responses") return passthroughResponses(args);
  if (args.profile && args.profile.claudeUpstreamProtocol === "responses-compact") return passthroughResponsesCompact(args);
  const body = parseJsonBody(args.body);
  const model = body.model || args.profile.model;
...
module.exports = {
  ...
  chatToAnthropic,
  chatToResponses,
  responsesToAnthropicPayload,
  proxyMessagesViaChat,
  proxyResponsesViaAnthropic,
  proxyResponsesViaAnthropicMessages,
