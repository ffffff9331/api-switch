...
const { errorCategory, isRetryableError, retryableStatusForFallback } = require("./stability");
...
async function proxyWithFallback({ routes, getApiKey, adapterForProfile, requestArgs, res, recordAttempt }) {
  ...
  for (let index = 0; index < routes.length; index += 1) {
    ...
    try {
      ...
      if (!retryableStatusForFallback(status) || index === routes.length - 1) {
        capture.flushTo(res);
...
async function proxyResponsesBody({ options, req, res, urlPath, client, profile, fallbackProfiles, body }) {
  ...
  const routeTargets = filterHealthyRouteTargets(buildRouteTargets({
    client,
    requestedModel: payloadModel,
    activeProfile: profile,
    fallbackProfiles,
    resolveModelRoute: options.resolveModelRoute || (() => null),
  }), options.isProfileAvailable);
      ...
      for (let index = 0; index < routeTargets.length; index += 1) {
        ...
        try {
          const args = {
            ...
            streamingFirstChunkTimeoutMs: streamingFirstChunkTimeoutMs(options),
          ...
          if (res.headersSent || index === routeTargets.length - 1 || !isRetryableError(error)) throw error;
        }
      }
...
function buildRouteTargets({ client, requestedModel, activeProfile, fallbackProfiles, resolveModelRoute }) {
  ...
  const upstreamModel = primary.mapped
    ? (primary.upstreamModel || profile.model || requestedModel)
    : (profile.model || requestedModel);
  const fallbacks = fallbackProfiles
    .filter((item) => item && item.name !== profile.name)
    .map((fallbackProfile) => ({
      profile: fallbackProfile,
      upstreamModel: fallbackProfile.model,
      requestedModel,
      mapped: false,
    }));
...
function filterHealthyRouteTargets(routeTargets, isProfileAvailable) {
  if (!Array.isArray(routeTargets) || routeTargets.length <= 1 || typeof isProfileAvailable !== "function") return routeTargets;
  const available = routeTargets.filter((target) => isProfileAvailable(target.profile && target.profile.name));
  return available.length ? available : routeTargets;
}

function streamingFirstChunkTimeoutMs(options) {
  const value = Number(options && options.streamingFirstChunkTimeoutMs);
